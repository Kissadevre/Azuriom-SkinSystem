<?php

return [
    'title' => 'My skin',
    'description' => 'Upload and preview the Minecraft skin managed by your Azuriom account.',
    'viewer' => [
        'eyebrow' => 'Live preview',
        'title' => '3D preview',
        'tip' => 'Drag to rotate',
        'canvas' => 'Interactive 3D preview of your Minecraft skin',
        'empty_title' => 'Your skin will appear here',
        'empty' => 'Choose a PNG skin to preview it here.',
        'loading' => 'Loading preview...',
        'error' => 'The selected skin could not be rendered in the 3D viewer.',
        'unsupported' => 'Your browser does not support the 3D skin viewer.',
    ],
    'upload' => [
        'eyebrow' => 'New appearance',
        'title' => 'Upload a skin',
        'help' => 'PNG only, 64×64 or legacy 64×32 pixels, up to 3 MB.',
        'drop_title' => 'Drop your Minecraft skin here',
        'drop_action' => 'or click to choose a PNG',
        'change' => 'Change',
        'requirements' => 'Skin file requirements',
        'variant_help' => 'Choose how the viewer and SkinsRestorer should interpret the arm width.',
        'sync_note' => 'Uploading activates and submits the skin to Minecraft. Saving only adds it to your library.',
        'invalid_type' => 'Choose a PNG image.',
        'invalid_size' => 'The PNG must not exceed 3 MB.',
        'name_placeholder' => 'For example: RedstoneHero',
    ],
    'current' => [
        'eyebrow' => 'Active skin',
        'tab' => 'Current skin',
        'title' => 'Current website skin',
    ],
    'capes' => [
        'label' => 'MineSkin cape',
        'help' => 'Optional. A cape uses the site\'s global MineSkin integration and may take a few seconds to generate.',
        'none' => 'No cape',
        'saved' => 'Saved cape',
        'empty' => 'MineSkin did not return any supported capes for the configured key.',
        'catalog_unavailable' => 'The MineSkin cape catalog is temporarily unavailable. You can still upload without a cape.',
    ],
    'delete' => [
        'title' => 'Delete skin',
        'confirm' => 'Are you sure you want to remove your current website skin?',
    ],
    'fields' => [
        'name' => 'Skin name',
        'skin' => 'Minecraft skin PNG',
        'variant' => 'Arm model',
        'resolved_variant' => 'Detected model',
        'revision' => 'Revision',
        'sync_status' => 'Server synchronization',
        'last_dispatched_at' => 'Last dispatch attempt',
        'cape' => 'Cape',
        'delivery' => 'Delivery',
    ],
    'delivery' => [
        'direct' => 'Direct URL',
        'mineskin' => 'MineSkin',
    ],
    'variants' => [
        'auto' => 'Detect automatically',
        'classic' => 'Classic (4-pixel arms)',
        'slim' => 'Slim (3-pixel arms)',
    ],
    'variant_descriptions' => [
        'auto' => 'Recommended',
        'classic' => '4-pixel arms',
        'slim' => '3-pixel arms',
    ],
    'actions' => [
        'upload' => 'Upload skin',
        'replace' => 'Replace skin',
        'delete' => 'Delete skin',
        'download' => 'Download PNG',
        'sync' => 'Synchronize again',
        'uploading' => 'Uploading...',
        'save' => 'Save skin',
        'activate' => 'Use skin',
        'remove_saved' => 'Remove from library',
    ],
    'status' => [
        'updated' => 'Your skin was saved successfully.',
        'unchanged' => 'This skin and arm model are already active on the website.',
        'deleted' => 'Your website skin was deleted.',
        'activated' => 'The selected skin is now active.',
        'already_active' => 'That skin is already active.',
        'saved_deleted' => 'The skin was removed from your library. Your active skin was not changed.',
        'saved' => 'The skin was saved to your library without changing your active skin.',
    ],
    'library' => [
        'eyebrow' => 'Personal collection',
        'title' => 'My skin library',
        'usage' => ':count of :limit saved',
        'empty_title' => 'Your library is empty',
        'empty' => 'Choose a PNG and use Save skin to add it here without activating it.',
        'active' => 'Active',
        'preview_alt' => 'Texture preview for :name',
        'delete_title' => 'Remove saved skin',
        'delete_confirm' => 'Remove “:name” from your library? This will not change the skin currently active in Minecraft.',
        'default_name' => 'Minecraft skin',
        'save_title' => 'Save to library',
        'save_help' => 'Give this skin a name. Saving it will not change your current website or Minecraft skin.',
        'name_help' => 'Up to 16 characters. Use letters and numbers only.',
        'limit_reached' => 'Your library has reached its limit of :limit skins. Choose one entry to replace.',
        'replace_label' => 'Skin to replace',
        'replace_placeholder' => 'Choose a saved skin',
    ],
    'sync' => [
        'status' => [
            'pending' => 'Pending',
            'submitted' => 'Submitted',
            'failed' => 'Failed',
            'uncertain' => 'Outcome unknown',
            'not_configured' => 'Not configured',
        ],
        'status_help' => 'Submitted means the command was handed to the configured bridge; it does not confirm that SkinsRestorer applied the skin.',
        'clear_state_title' => 'SkinsRestorer clear operation',
        'clear_state_help' => 'The website skin is deleted. This state is retained so clear commands for every recorded server destination can be submitted again.',
        'feedback' => [
            'submitted' => 'The SkinsRestorer command was submitted.',
            'clear_submitted' => 'The SkinsRestorer clear command or commands were submitted.',
        ],
        'errors' => [
            'sync_disabled' => 'Server synchronization is currently disabled by an administrator.',
            'server_unavailable' => 'The configured Minecraft server is unavailable or cannot execute commands.',
            'invalid_game_id' => 'Your account does not contain a valid Minecraft UUID, so the server command was not submitted.',
            'invalid_variant' => 'The resolved arm model is invalid. Upload the skin again before retrying.',
            'invalid_public_url' => 'The generated public skin URL is invalid or contains unsafe characters.',
            'insecure_public_url' => 'The public skin URL must use HTTPS before it can be submitted to SkinsRestorer.',
            'unreachable_public_url' => 'The public skin URL uses a local or private host that MineSkin cannot reach.',
            'public_url_too_long' => 'The public skin URL exceeds the length supported by SkinsRestorer.',
            'queue_dispatch_failed' => 'SkinSystem could not save the SkinsRestorer command in the AzLink queue. Check the database connection before trying again.',
            'queue_cleanup_failed' => 'SkinSystem could not safely replace an older queued SkinsRestorer command. Try again after checking the database connection.',
            'dispatch_exception' => 'The bridge returned an error after dispatch began. Check the Minecraft server before synchronizing again.',
            'clear_may_be_in_flight' => 'A previous clear command may already be executing. The new skin command was submitted, but its final order cannot be confirmed; verify it in game and synchronize again if needed.',
            'stale_revision' => 'A newer skin revision replaced this request before it could be submitted. Reload the page and try again if needed.',
            'operation_busy' => 'Another skin operation is still running for your account. Wait a moment and try again.',
            'mineskin_processing' => 'MineSkin is generating this appearance. This page will update automatically when it is ready.',
            'mineskin_unavailable' => 'MineSkin is temporarily unavailable. SkinSystem will retry the queued operation automatically.',
            'mineskin_rate_limited' => 'The global MineSkin key reached its current rate limit. SkinSystem will retry automatically.',
            'mineskin_invalid_key' => 'MineSkin rejected the configured API key. Ask an administrator to verify the integration.',
            'mineskin_request_rejected' => 'MineSkin rejected this skin or cape generation request.',
            'mineskin_invalid_response' => 'MineSkin returned an unexpected response. The appearance was not submitted to Minecraft.',
            'mineskin_generation_failed' => 'MineSkin could not generate this skin and cape combination. You can retry from Current skin.',
            'mineskin_not_configured' => 'This appearance requires MineSkin, but no global API key is currently configured.',
            'mineskin_source_missing' => 'The source PNG for this MineSkin operation is no longer available.',
        ],
    ],
    'validation' => [
        'invalid_skin' => 'The uploaded file is not a valid, decodable PNG skin.',
        'dimensions' => 'The skin must be exactly 64×64 or 64×32 pixels.',
        'legacy_slim' => 'Legacy 64×32 skins only support the classic arm model.',
        'gd_missing' => 'The server cannot process skins because the PHP GD extension is unavailable.',
        'library_full' => 'Your skin library is full (maximum :limit). Remove a saved skin before uploading another one.',
        'name_required' => 'Enter a name for this skin.',
        'name_max' => 'The skin name may not contain more than 16 characters.',
        'name_format' => 'The skin name may contain letters and numbers only.',
        'replacement_required' => 'Choose which saved skin should be replaced.',
        'replacement_invalid' => 'The selected saved skin does not belong to your library.',
        'cape_unavailable' => 'Cape selection is not available for your account or the current SkinSystem configuration.',
        'cape_invalid' => 'Choose a currently supported MineSkin cape.',
    ],
];
